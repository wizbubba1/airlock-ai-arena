...
`npm run randomness-beacon-plan -- <seed>` writes JSON and Markdown randomness-beacon plan artifacts for future drand role assignment, per-tick entropy, and audit publication timing.
`npm run verify-randomness-beacon-plan -- <randomness-beacon-plan.json>` reconstructs the plan and fails if entropy timing policy, entries, ruleset, or plan hash drift.
